...
import org.firstinspires.ftc.teamcode.commandbase.teleopCommands.pick_testttt;
...
//@Disabled
@Config
@TeleOp(name = "FOC test")
public class FOC_test extends CommandOpMode {
...
//        schedule(new intake_init(intake,outake));
    ...
    @Override
    public void run() {
        robot.drivetrain.set(new Pose(gamepad1.left_stick_x, -gamepad1.left_stick_y, MathUtils.joystickScalar(-gamepad1.right_stick_x , 0.01)), 0);
...
//        }
        ...
        if (RIGHT){
            schedule(new pick_testttt(intake,outake));
//            schedule(new intake_pickBack2Pos(intake,outake));
//            drone.
        }
        if (gamepad1.a){
...
//        }
        ...
        telemetry.addData("left front", robot.leftFront.getCurrent(CurrentUnit.AMPS));
        telemetry.addData("right front", robot.rightFront.getCurrent(CurrentUnit.AMPS));
        telemetry.addData("left back", robot.leftRear.getCurrent(CurrentUnit.AMPS));
        telemetry.addData("right back", robot.rightRear.getCurrent(CurrentUnit.AMPS));
