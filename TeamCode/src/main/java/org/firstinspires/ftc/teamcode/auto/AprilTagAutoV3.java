...
public class AprilTagAutoV3 extends CommandOpMode{
    ...
    public static double SPEED_GAIN  =  0.09; // TODO ==== CHANGE THESE VALUES ====  Forward Speed Control "Gain". eg: Ramp up to 50% power at a 25 inch error.   (0.50 / 25.0)
    ...
    public void runOpMode() throws InterruptedException {
        ...
        startHeadig = drive.getPoseEstimate().getHeading();
...
//        TODO:CENTRE3
        TrajectorySequence centre3 = drive.trajectorySequenceBuilder(centre2.end())
                ...
                .lineToConstantHeading(new Vector2d(-10,43.4))
                .addDisplacementMarker(() -> {;
        ...
        while (opModeIsActive()) {
            ...
            try {
                if(desiredTag.metadata != null){
                    ...
                    telemetry.addData("x before update",drive.getPoseEstimate().getX());
                    telemetry.addData("y before update",drive.getPoseEstimate().getY());

                    Pose2d newPose = new Pose2d(drive.getPoseEstimate().getX(), drive.getPoseEstimate().getY(), drive.getPoseEstimate().getHeading());
                    drive.setPoseEstimate(newPose);
                    drive.update();
                    telemetry.addData("x",drive.getPoseEstimate().getX());
                    telemetry.addData("y",drive.getPoseEstimate().getY());

                    TrajectorySequence BackdropPose = drive.trajectorySequenceBuilder((newPose))
                            .lineToConstantHeading(new Vector2d(29.5, 43.4)) // TODO CHANGE THIS ACCORDING TO THE ROBOT CURRENT POSE
                            .build();
                    drive.followTrajectorySequence(BackdropPose);
                    telemetry.addData("x after odo",drive.getPoseEstimate().getX());
                    telemetry.addData("y after odo",drive.getPoseEstimate().getY());
                    telemetry.update();
                }
                else {
                    TrajectorySequence correct = drive.trajectorySequenceBuilder(new Pose2d(drive.getPoseEstimate().getX(), drive.getPoseEstimate().getY(), drive.getPoseEstimate().getHeading()))
                            .lineToLinearHeading(new Pose2d(drive.getPoseEstimate().getX(),drive.getPoseEstimate().getY(), 0))
                            .build();
                ...
                sleep(200);
                    ...
                    } else if (heading < Math.toRadians(0)) {
                        drive.turn(Math.toRadians(20));
