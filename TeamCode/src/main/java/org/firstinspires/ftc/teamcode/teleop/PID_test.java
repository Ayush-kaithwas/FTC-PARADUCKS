...
//import com.acmerobotics.roadrunner.util.PIDCoefficients;
...
public class PID_test extends CommandOpMode {
    ...
    private PIDCoefficients headingPidCoefficients = new PIDCoefficients(kp, ki, kd);
    private double targetHeading = 0.0;

    // Add this method for heading control
...
//            Vector2d input = new Vector2d(Math.pow(Range.clip(gamepad1.left_stick_y, -1, 1), 3), Math.pow(Range.clip(gamepad1.left_stick_x, -1, 1), 3)).rotated(-poseEstimate.getHeading());
        ///////ROBOT ORIENTED/////////
        Vector2d input = new Vector2d(Math.pow(Range.clip(gamepad1.left_stick_y, -1, 1), 3), Math.pow(Range.clip(gamepad1.left_stick_x, -1, 1), 3));
        double headingError = Angle.normDelta(targetHeading - currentHeading);
        double correction = headingPidCoefficients.p * headingError;
        drive.setWeightedDrivePower(
                new Pose2d(
                        input.getX() * Xvalue,
                        input.getY() * Yvalue,
                        (-gamepad1.right_stick_x * 0.8) + correction));
